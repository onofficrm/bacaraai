...
$admin_id = isset($config['cf_admin']) ? sql_real_escape_string($config['cf_admin']) : '';
$where = " where (mb_leave_date = '' or mb_leave_date is null) ";
if ($admin_id !== '') {
    $where .= " and mb_id <> '{$admin_id}' ";
}
if ($stx !== '') {
...
<?php bacara_wallet_admin_shell_start('회원 · 가상머니 관리', '공개 회원가입은 막혀 있습니다. 최고관리자가 아이디·비밀번호를 부여하고 가상 게임머니(시드)를 지급하세요.'); ?>

<p class="bw-notice">가상머니는 그누보드 포인트와 별도입니다. AI 과금 포인트와 섞이지 않습니다.</p>
...
<div class="bw-card">
    <form method="get" action="" class="bw-search">
        ...
        <button type="submit" class="btn_submit bw-ghost">검색</button>
        <a href="<?php echo htmlspecialchars(bacara_wallet_admin_url('form.php'), ENT_QUOTES, 'UTF-8'); ?>" class="btn_submit">+ 회원 생성</a>
    </form>
                ...
                for ($i = 0; $row = sql_fetch_array($result); $i++) {
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <span style="color:#71717a"> / <?php echo htmlspecialchars($row['mb_nick'], ENT_QUOTES, 'UTF-8'); ?></span>
                        </td>
